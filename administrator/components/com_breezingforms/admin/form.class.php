	...
	static function save($option, $pkg, $caller, $nonclassic, $quickmode)
	{
		...
		if(!$nonclassic){
			JFactory::getApplication()->redirect($caller, BFText::_('COM_BREEZINGFORMS_FORMS_SAVED'));
                    ...
                    if( (isset($_POST['reload_flag']) && $_POST['reload_flag'] == 1) || ( $_POST['salesforce_flag'] == 1 && $_POST['salesforce_enabled'] == 1 ) ){
                        ...
                        if(!$quickmode){
                                JFactory::getApplication()->redirect('index.php?option=com_breezingforms&format=html&act=easymode&formName='.$row->name.'&form='.$row->id);
                        } else {
                                JFactory::getApplication()->redirect('index.php?option=com_breezingforms&act=quickmode&formName='.$row->name.'&form='.$row->id);
                        }	
                    }
		}
	} // save
