	...
	public function process(&$dataObject, $parent = null, $parentPage = null, $index = 0, $childrenLength = 0, $parentFull = null){
		if(isset($dataObject['attributes']) && isset($dataObject['properties']) ){
			...
			if($mdata['type'] == 'page'){
				...
				echo '<nav id="bfPage'.$parentPage['pageNumber'].'" class="bfPage"'.$display.'>'."\n"; // opening current page
				...
				if(!$mdata['hideLabel']){
					...
					if(trim($mdata['hint']) != ''){
						...
						$content = trim($mdata['hint']);
                                                ...
                                                $tipOpen   = '<div style="display: inline;" class="hasTooltip" title="'.JHtml::tooltipText($content).'">';
						$tipClose  = '</div>';
                                            }
                                        }
                                        
                                        if($tipOpen && (!isset($this->rootMdata['joomlaHint']) || !$this->rootMdata['joomlaHint'])){
                                            $tipOpen = $tipOpen.'<i class="icon-question-sign hidden-phone hidden-tablet">&nbsp;</i> ';
                                        }
					...
					$label = '<label class="control-label'.(isset($this->rootMdata['themebootstrapLabelTop']) && $this->rootMdata['themebootstrapLabelTop'] ? ' bfLabelBlock' : '').'" id="bfLabel'.$mdata['dbId'].'" '.$for.'>'.$required.$tipOpen.str_replace("***","\"",$labelText).$tipClose.'</label>'."\n";
				}
				...
				switch($mdata['bfType']){
						...
						if($mdata['group'] != ''){
							...
							for($i = 0; $i < $lines; $i++){
								...
								if($iCnt == 3){
									...
									echo htmlentities(trim($iEx[1]), ENT_QUOTES, 'UTF-8').'</label>'.($i + 1 < $lines && $mdata['wrap'] ? '<div style="clear:both;"></div>' : '');
								}
							}
                                                        if($mdata['wrap']){
                                                            echo '</div>';
						...
						if( ( isset( $mdata['flashUploader'] ) && $mdata['flashUploader'] ) || ( isset( $mdata['html5'] ) && $mdata['html5'] ) ){
							...
							echo "
                                                        <span id=\"bfUploadContainer".$mdata['dbId']."\">
                                                            <button class=\"btn bfUploadButton button\" id=\"bfPickFiles".$mdata['dbId']."\"><i class=\"icon-upload\"></i></button>
							</span>
                                                                ...
                                                                uploader.bind('FilesAdded', function(up, files) {
                                                                        ...
                                                                        for (var i in files) {
                                                                            if(typeof files[i].id != 'undefined' && files[i].id != null){
                                                                                ...
                                                                                JQuery('#bfFlashFileQueue".$mdata['dbId']."').append('<div class=\"bfFileQueueItem\" id=\"' + files[i].id + 'queueitem\"><div id=\"' + files[i].id + 'thumb\"></div><div id=\"' + files[i].id + '\"><img id=\"' + files[i].id + 'cancel\" src=\"".$this->cancelImagePath."\" style=\"cursor: pointer; padding-right: 10px;\" border=\"0\"/>' + (iOS ? '' : files[i].name ? files[i].name.substring(0,12) : '') + ' ' + fsize + '<b id=\"' + files[i].id + 'msg\" style=\"color:red;\"></b></div></div>');
                                                                                var file_ = files[i];
					...
					case 'bfSubmitButton':
						...
						if($mdata['actionClick'] == 1){
							...
							$onclick = 'onclick="if(typeof bf_htmltextareainit != \'undefined\'){ bf_htmltextareainit() }populateSummarizers();if(document.getElementById(\'bfPaymentMethod\')){document.getElementById(\'bfPaymentMethod\').value=\'\';};return false;" ';
                                                ...
                                                if($src == ''){
                                                    echo '<button class="ff_elem btn button" '.$value.$src.$tabIndex.$onclick.$onblur.$onchange.$onfocus.$onselect.$readonly.'type="'.$type.'" name="ff_nm_'.$mdata['bfName'].'[]" id="ff_elem'.$mdata['dbId'].'">'.$mdata['value'].'</button>'."\n";
                                                }else{
                                                    echo '<input class="ff_elem btn button" '.$value.$src.$tabIndex.$onclick.$onblur.$onchange.$onfocus.$onselect.$readonly.'type="'.$type.'" name="ff_nm_'.$mdata['bfName'].'[]" id="ff_elem'.$mdata['dbId'].'" value="'.$mdata['value'].'"/>'."\n";
                                                }
                                                echo '</div>';
						break;
					...
					case 'bfCaptcha':
                                                ...
                                                echo '<img border="0" '.(isset($mdata['width']) && intval($mdata['width']) > 0 ? ' style="width: '.intval($mdata['width']).'px !important;min-width: '.intval($mdata['width']).'px !important;max-width: '.intval($mdata['width']).'px !important;"' : 'style="width: 230px !important;min-width: 230px !important;max-width: 230px !important;"' ).' id="ff_capimgValue" class="ff_capimg img-polaroid" src="'.$captcha_url.'"/>'."\n";
                                                echo '<br/>';
                                                echo '<br/>';
						...
						echo '<button class="ff_elem btn button" onclick="document.getElementById(\'bfCaptchaEntry\').value=\'\';document.getElementById(\'bfCaptchaEntry\').focus();document.getElementById(\'ff_capimgValue\').src = \''.$captcha_url.'?bfMathRandom=\' + Math.random(); return false"><i class="icon-refresh"></i></button>'."\n";
						echo '</div>';
                                                echo '</div>';
					...
					case 'bfCalendar':
						...
						echo '<button id="ff_elem'.$mdata['dbId'].'_calendarButton" type="submit" class="bfCalendar btn button" value="'.htmlentities($right, ENT_QUOTES, 'UTF-8').'"><span><i class="icon-calendar"></i>'.htmlentities($right == '...' ? '' : $right, ENT_QUOTES, 'UTF-8').'</span></button>'."\n";
						echo '</div>'."\n";
		...
		if(isset($dataObject['properties']) && $dataObject['properties']['type'] == 'section' && $dataObject['properties']['bfType'] == 'section'){
			...
			if(!$isLastPage){
				...
				if($this->rootMdata['pagingInclude'] && $dataObject['properties']['pageNumber'] > 1){
					echo '<button class="bfPrevButton btn btn-primary pull-left button'.$this->fadingClass.'" type="submit" onclick="if(ff_currentpage > 1){ff_switchpage(ff_currentpage-1);self.scrollTo(0,0);}populateSummarizers();if(typeof bfRefreshAll != \'undefined\'){bfRefreshAll();}" value="'.htmlentities(trim($this->rootMdata['pagingPrevLabel']), ENT_QUOTES, 'UTF-8').'"><span>'.htmlentities(trim($this->rootMdata['pagingPrevLabel']), ENT_QUOTES, 'UTF-8').'</span></button>'."\n";
				}
	
				if($this->rootMdata['pagingInclude'] && $dataObject['properties']['pageNumber'] < count($this->dataObject['children']) - $last){
					echo '<button class="bfNextButton btn btn-primary pull-right button'.$this->fadingClass.'" type="submit" onclick="ff_validate_nextpage(this, \'click\');populateSummarizers();if(typeof bfRefreshAll != \'undefined\'){bfRefreshAll();}" value="'.htmlentities(trim($this->rootMdata['pagingNextLabel']), ENT_QUOTES, 'UTF-8').'"><span>'.htmlentities(trim($this->rootMdata['pagingNextLabel']), ENT_QUOTES, 'UTF-8').'</span></button>'."\n";
				}
