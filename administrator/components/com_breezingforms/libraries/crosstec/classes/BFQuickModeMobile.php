			...
			else if($mdata['type'] == 'element')
			{
				...
				switch($mdata['bfType'])
				{
					...
					case 'bfTextarea':
						...
						echo '<textarea '.(isset($mdata['placeholder']) && $mdata['placeholder'] ? 'placeholder="'.htmlentities($mdata['placeholder'], ENT_QUOTES, 'UTF-8').'" ' : '').'cols="20" rows="5" class="ff_elem" '.$onkeyup.$tabIndex.$onclick.$onblur.$onchange.$onfocus.$onselect.$readonly.'name="ff_nm_'.$mdata['bfName'].'[]" id="ff_elem'.$mdata['dbId'].'">'.htmlentities(trim($mdata['value']), ENT_QUOTES, 'UTF-8').'</textarea>'."\n";
					break;
					...
					case 'bfFile':
						if((isset($mdata['flashUploader']) && $mdata['flashUploader']) || (isset($mdata['html5']) && $mdata['html5']))
						{
							...
							$flashUploader = "<span id=\"bfUploadContainer".$mdata['dbId']."\">
		...
		if($this->toggleFields != '[]')
		{
			$toggleCode = '
				...
				String.prototype.beginsWith = function(t, i)
				{
					...
					{
						return
						(t.toLowerCase() == this.substring(0, t.length).toLowerCase());
					}
				}
													...
													for(var k = 0; k < thisToggleFieldsArray.length; k++)
													{
														...
														if(thisToggleFieldsArray[k].condition == "isnot")
														{
															...
															if(
																...
																||
																(JQuery(this).get(0).type == "checkbox" && JQuery(this).get(0).name == "ff_nm_"+thisToggleFieldsArray[k].sName+"[]" && (JQuery(this).get(0).checked && thisToggleFieldsArray[k].value != "!checked" || JQuery(this).get(0).checked && thisToggleFieldsArray[k].value == "!unchecked"))
																||
