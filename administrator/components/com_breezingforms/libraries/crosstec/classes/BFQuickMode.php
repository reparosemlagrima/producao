								...
								if(element.get(0))
								{
									switch(element.get(0).type)
									{
										...
										case "textarea":
											...
											JQuery("[name=\"ff_nm_"+toggleField.sName+"[]\"]").blur(function()
											{
												for(var k = 0; k < thisToggleFieldsArray.length; k++)
												{
													...
													{
														if(
															(regExp != "" && testRegExp.test(JQuery(this).val()) == false)
															||
															JQuery(this).val() != thisToggleFieldsArray[k].value)
															&&
										...
										case "select-one":
											...
											JQuery("[name=\"ff_nm_"+toggleField.sName+"[]\"]").change(function()
											{
						...
						if(JQuery.bfvalidationEngine)
						{
							...
							for(var i = 0; i < inlineErrorElements.length; i++)
							{
								if(inlineErrorElements[i][1] != "")
								{
									...
									JQuery(prompt).mouseover(function()
									{
	...
	public function process(&$dataObject, $parent = null, $parentPage = null, $index = 0, $childrenLength = 0)
	{
		if(isset($dataObject['attributes']) && isset($dataObject['properties']))
		{
			...
			if($mdata['type'] == 'page')
			{
				...
				echo '<div id="bfPage'.$parentPage['pageNumber'].'" class="bfPage" '.(JRequest::getInt('ff_page',1) == $parentPage['pageNumber'] && JRequest::getInt('ff_form_submitted',0) == 0 ? ''  : ( JRequest::getInt('ff_form_submitted',0) == 1 && $this->rootMdata['lastPageThankYou'] && $parentPage['pageNumber'] == count($this->dataObject['children']) ? '' :  'style="display:none"') ).'>'."\n"; // opening current page
				...
				if(!$mdata['hideLabel'])
				{
					...
					if(trim($mdata['hint']) != '')
					{
						...
						if(isset($this->rootMdata['joomlaHint']) && $this->rootMdata['joomlaHint'])
						{
							...
							$tipOpen   = '<span title="'.addslashes(trim($mdata['label'])).'::'.str_replace( array("\n","\r"), array("",""), htmlentities($content, ENT_QUOTES, 'UTF-8')).'" id="bfTooltip'.$mdata['dbId'].'" class="bfTooltip editlinktip hasTip"><span class="bfTooltip">';
							$tipClose  = '</span></span>';
							$tipScript = '';
						}
						else
				...
				switch($mdata['bfType'])
				{
					...
					case 'bfTextarea':
						...
						if(isset($mdata['is_html']) && $mdata['is_html'])
						{
							...
							echo '<div>'.$editor->display('ff_nm_'.$mdata['bfName'].'[]',htmlentities(trim($mdata['value']), ENT_QUOTES, 'UTF-8'), strip_tags($mdata['width']), strip_tags($mdata['height']), '75', '20').'</div>';
						}
						else
						{
