...
class KunenaAdminModelUsers extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		//filtrar por thanks
		$filter_active .= $value = $this->getUserStateFromRequest($this->context . '.filter.thankyou', 'filter_thankyou', '', 'string');
		$this->setState('filter.thankyou', $value);
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.thankyou');
	...
	protected function getListQuery()
	{
		...
		// Filter by thank.
		$thankyou = $this->getState('filter.thankyou');

		if (!empty($thankyou))
		{
			
			$query->where('ku.thankyou = ' . (int)$thankyou);
		}
		...
		switch ($this->state->get('list.ordering'))
		{
			...
			case 'name':
				...
				break;
			case 'thankyou':
				$query->order('ku.thankyou ' . $direction);
				break;
			case 'username':
			default:
