							...
							<div class="tab-content">
								<div class="tab-pane active" id="tab1">
									<div class="row-fluid">
										...
										<div class="span9">
											<fieldset>
												<legend>Suportes atendidos com sucesso: <span class="badge"><?=$this->user->thankyou?></span></legend>
												<div class="">
													<h4>Contagem dos últimos 7 dias: <span class="badge"><?=$this->thanks;?></span></h4>
													<h4>Karma atribuido pelos usuários: <span class="badge"><?=$this->user->karma;?></span></h4>
													<h4>Enviar Cupom de desconto para este usuário </h4>
													<input type="text" value="" name="cupom" id="cupom" placeholder="cod. do cupom" /> <button class="btn btn-primary" id="addcupom">Enviar cupom</button>
													<p style="margin:10px;"><a target="_blank" class="btn btn-danger" href="<?php echo JURI::root(); ?>loja/admin/index.php?route=marketing/coupon&token=r96zq5AAdAQmXz6kmjnF84NxFdH1nMnC">Gerar cupom de desconto</a></p>
													<div id="errosmail">
													</div>
												</div>
											</fieldset>
									...
									<fieldset>
										...
										<script type="text/javascript">
										 ...
										 and(document).ready(function(){
										 	...
										 	and("#addcupom").on("click",function(e){
										 		e.preventDefault();
											 		and.ajax({
											 		url:'index.php?option=com_kunena&view=ajax',
											 		method:'POST',
											 		datatype:'json',
											 		data: {"email":"<?=$this->user->email; ?>","code":and("#cupom").val()},
											 		beforeSend: function(){
											 			and("#addcupom").attr("disabled");
											 			and("#addcupom").text("aguarde...");
											 		},
											 		success:function(data){
											 			data = JSON.parse(data);

											 			if(data.erro){
											 				and("#errosmail").html("<div class='alert alert-danger'>"+data.msn+"</div>").delay(3000).hide(1500);
											 				and("#addcupom").text("erro");
											 				and("#addcupom").attr("disabled","false");
											 			}else{
											 				and("#errosmail").html("<div class='alert alert-success'>"+data.msn+"</div>").delay(3000).hide(1500);
											 				and("#addcupom").text("Enviado");
											 			}
											 		}
											 	});
