...
<div class="kblock">
	<div class="kheader">
		...
		<br/>
		<p class="better_questions">Seja específico e inclua o que você já tentou. <a href="/fazendo-otimas-perguntas" title="Melhores perguntas tem mais respostas" target="_blank">Melhores perguntas tem mais respostas</a>.</p>
	</div>
	<div class="kcontainer">
		<div class="kbody">
			<?php if(isset($this->selectcatlist)): ?>
				<div id="kpost-category" class="line_form_topic">
					<label><strong><?php echo JText::_('COM_KUNENA_CATEGORY')?></strong></label>
					<?php echo $this->selectcatlist; ?>
				</div>
			<?php endif; ?>

			<?php if(!$this->message->userid): ?>
				<?php $style = (!$this->category->allow_anonymous) ? ' style="display:none;"' : ''; ?>
				<div id="kanynomous-check" class="line_form_topic"<?php echo $style; ?>>
					<label for="kanonymous"><strong><?php echo JText::_('COM_KUNENA_POST_AS_ANONYMOUS'); ?></strong></label>
			...
			<?php $style = ($this->me->userid && !$this->category->allow_anonymous) ? ' style="display:none;' : ''; ?>
			<div id="kanynomous-check-name" class="line_form_topic"<?php echo $style; ?>>
				<label for="kauthorname"><strong><?php echo JText::_('COM_KUNENA_GEN_NAME'); ?></strong></label>
			...
			<?php if ($this->config->askemail && !$this->me->userid) : ?>
				<div class="line_form_topic">
					<label for="email"><strong><?php echo JText::_('COM_KUNENA_GEN_EMAIL');?></strong></label>
			...
			<?php // COMEÇO OFICIAL FORMULÁRIO TÓPICO ?>

			<?php if(!empty($this->topicIcons)): ?>
				<div id="kpost-topicicons" class="line_form_topic">
					<label><strong><?php echo JText::_('COM_KUNENA_GEN_TOPIC_ICON'); ?></strong></label>
			...
			<div id="kpost-subject" class="line_form_topic">
				<label for="subject"><strong><?php echo JText::_('COM_KUNENA_GEN_SUBJECT'); ?></strong></label>
			...
			<?php if($this->allowedExtensions): ?>
				<div id="kpost-attachments" class="line_form_topic">
					<label><strong><?php echo JText::_('COM_KUNENA_EDITOR_ATTACHMENTS') ?></strong></label>
			...
			<?php if($this->config->keywords && $this->me->isModerator($this->topic->getCategory())): ?>
				<div id="kpost-tags" class="line_form_topic">
					<label><strong><?php echo JText::_('COM_KUNENA_EDITOR_TOPIC_TAGS') ?></strong></label>
			...
			<?php if ($this->config->userkeywords && $this->me->userid) : ?>
				<div id="kpost-tags" class="line_form_topic">
					<label><strong><?php echo JText::_('COM_KUNENA_EDITOR_TOPIC_TAGS_OWN') ?></strong></label>
			...
			<?php if (!empty($this->captchaHtml)) : ?>
				<div id="kpost-captcha" class="line_form_topic">
					<td class="kcol-first">
			...
			<div id="kpost-buttons" class="line_form_topic">
