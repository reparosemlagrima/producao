...
<div id="kpost-toolbar" class="line_form_topic">
	...
	<div id="big_small_textarea">
		<span class="ks msg_expandir" onclick="kGrowShrinkMessage(100);" style="cursor: pointer"><?php echo JText::_('COM_KUNENA_EDITOR_ENLARGE'); ?></span>
		<span class="ks msg_contrair" onclick="kGrowShrinkMessage(-100);" style="cursor: pointer"><?php echo JText::_('COM_KUNENA_EDITOR_SHRINK'); ?></span>	
	</div>
	...
	<table class="kpostbuttonset">
		...
		<tr>
			<td class="kpostbuttons">
				...
				<div id="kbbcode-size-options" style="display: none;">
					<span style="display: none;" class="kmsgtext-xs" title='[size=1]' onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_FONTSIZE_XS', true);?>')">&nbsp;<?php echo JText::_('COM_KUNENA_EDITOR_SIZE_SAMPLETEXT'); ?>&nbsp;</span>
					<span class="kmsgtext-s" title='[size=2]' onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_FONTSIZE_S', true);?>')">&nbsp;<?php echo JText::_('COM_KUNENA_EDITOR_SIZE_SAMPLETEXT'); ?>&nbsp;</span>
					<span class="kmsgtext-m" title='[size=3]' onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_FONTSIZE_M', true);?>')">&nbsp;<?php echo JText::_('COM_KUNENA_EDITOR_SIZE_SAMPLETEXT'); ?>&nbsp;</span>
					<span class="kmsgtext-l" title='[size=4]' onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_FONTSIZE_L', true);?>')">&nbsp;<?php echo JText::_('COM_KUNENA_EDITOR_SIZE_SAMPLETEXT'); ?>&nbsp;</span>
					<span class="kmsgtext-xl" title='[size=5]' onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_FONTSIZE_XL', true);?>')">&nbsp;<?php echo JText::_('COM_KUNENA_EDITOR_SIZE_SAMPLETEXT'); ?>&nbsp;</span>
					<span style="display: none;" class="kmsgtext-xxl" title='[size=6]' onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_FONTSIZE_XXL', true);?>')">&nbsp;<?php echo JText::_('COM_KUNENA_EDITOR_SIZE_SAMPLETEXT'); ?>&nbsp;</span>
				</div>
				...
				<div id="kbbcode-link-options" style="display: none;">
					...
					<input id="kbbcode-link_url" name="url" type="text" size="40" value="http://" onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_LINKURL', true); ?>')" />
					<?php echo JText::_('COM_KUNENA_EDITOR_LINK_TEXT'); ?>&nbsp;
					<input name="text2" id="kbbcode-link_text" type="text" size="30" maxlength="150" onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_LINKTEXT', true); ?>')" />
					<input type="button" name="insterLink" value="<?php echo JText::_('COM_KUNENA_EDITOR_INSERT'); ?>" onclick="kbbcode.focus().replaceSelection('[url=' + document.id('kbbcode-link_url').get('value') + ']' + document.id('kbbcode-link_text').get('value') + '[/url]', false); kToggleOrSwap('kbbcode-link-options');" onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_LINKAPPLY', true); ?>')" />
				</div>
				...
				<!-- ENQUETE -->
				<?php
					if(!$this->message->parent && isset($this->poll)):
				?>
						<div id="kbbcode-poll-options" style="display: none;">
							...
							<input type="text" class="inputbox" name="poll_title" id="kpoll-title" maxlength="100" size="40" value="<?php echo $this->escape( $this->poll->title ) ?>" onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_POLLTITLE', true); ?>')" />

							<img id="kbutton-poll-add" src="<?php echo $this->ktemplate->getImagePath('icons/poll_add_options.png') ?>" onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_ADDPOLLOPTION', true); ?>')" alt="<?php echo JText::_('COM_KUNENA_POLL_ADD_POLL_OPTION'); ?>" />
							...
							<?php
								echo JHtml::_('calendar', isset($this->poll->polltimetolive) ? $this->escape($this->poll->polltimetolive) : '0000-00-00', 'poll_time_to_live', 'kpoll-time-to-live', '%Y-%m-%d',array('onmouseover'=>'document.id(\'helpbox\').set(\'value\', \''.JText::_('COM_KUNENA_EDITOR_HELPLINE_POLLLIFESPAN', true).'\')'));
							?>
				...
				?>
						<div id="kbbcode-code-options" style="display: none;">
							<?php echo $codeTypes; ?>
							<input id="kbutton_addcode" type="button" name="Code" onclick="kInsertCode()" value="<?php echo JText::_('COM_KUNENA_EDITOR_CODE_INSERT'); ?>"	onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_CODEAPPLY', true); ?>')" />
						</div>
				<?php
					endif;
				?>
						...
						<div id="kbbcode-video-options" style="display: none;">
							...
							<select id="kvideoprovider" name="provider" class="kbutton" onmouseover="document.id('helpbox').set('value', '<?php echo JText::_('COM_KUNENA_EDITOR_HELPLINE_VIDEOPROVIDER', true); ?>')">
								...
								<?php
									...
									$vid_provider = array ('', 'Clipfish', 'Dailymotion', 'DivX,divx]http://', 'Flash,flash]http://', 'FlashVars,flashvars param=]http://', 'MediaPlayer,mediaplayer]http://', 'QuickTime,quicktime]http://', 'RealPlayer,realplayer]http://', 'Videojug', 'Vimeo', 'Wideo.fr', 'YouTube');
		...
		<tr>
			<td class="kposthint">
				<input type="text" name="helpbox" id="helpbox" size="45" class="kinputbox" disabled="disabled" maxlength="100"
...
<div id="kpost-message" class="line_form_topic">
	<textarea class="ktxtarea required" name="message" id="kbbcode-message" rows="10" cols="50" tabindex="3">
		<?php echo $this->escape($this->message->message); ?>
	</textarea>

	<!-- Hidden preview placeholder -->
	<div id="kbbcode-preview" style="display: none;"></div>
	<?php
		if($this->message->exists() && $this->config->editmarkup):
	?>
			<div class="clr"> </div>
			<fieldset>
