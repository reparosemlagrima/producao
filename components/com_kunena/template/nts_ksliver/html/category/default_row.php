...
?>
	<li class="item_lista_forum">
		...
		<div class="autor_tempo_forum">
			<p>
				<?php
					echo $this->topic->getLastPostAuthor()->getLink(null, null, 'nofollow', '', null, $this->topic->getCategory()->id);
				?>
			</p>
			<p title="<?php echo KunenaDate::getInstance($this->topic->last_post_time)->toKunena('config_post_dateformat_hover'); ?>">
		...
		<div class="foto_forum">
			<?php
				if(!empty($this->topic->avatar)):
			?>
					<?php echo $this->topic->getLastPostAuthor()->getLink($this->topic->avatar, null, 'nofollow', '', null, $this->topic->getCategory()->id) ?>
			<?php
				endif;
			?>
		</div>
		...
		<div class="qtd_posts_forum">
			<?php
				$total_respostas = $this->formatLargeNumber(max(0,$this->topic->getTotal()-1));
				$class_respostas = ($total_respostas == 0) ? "no_answer" : "answer";
			?>
			<span class="<?php echo $class_respostas; ?>"><?php echo $total_respostas; ?></span>
		</div>
...
?>
	...
	<tr class="<?php echo $this->getTopicClass('k', 'row') ?>">
			...
			<div class="ktopic-details">
				...
				<div class="ktopic-details-kcategory">
					...
					<span class="ktopic-by ks">
						<?php echo JText::_('COM_KUNENA_BY').' '.$this->topic->getFirstPostAuthor()->getLink(null, null, 'nofollow', '', null, $this->topic->getCategory()->id) ?>
					</span>
				</div>
				...
				<div class="ktopic-details-kcategory" style="clear:both;">
					<?php if($this->pages > 1) : ?>
						<ul class="kpagination">
							<li class="page">
								<?php echo JText::_('COM_KUNENA_PAGE') ?>
							</li>
							<li>
								<?php echo $this->GetTopicLink($this->topic, 0, 1) ?>
							</li>
							<?php
								if($this->pages > 4):
									$startPage = $this->pages - 3;
							?>
								<li class="more">...</li>
							<?php
								else:
							...
							?>
									<li>
										<?php echo $this->getTopicLink($this->topic, $hopPage, $hopPage+1) ?>
									</li>
							<?php
								endfor;
							?>
		...
		<td class="kcol-first kcol-ktopicreplies hidden-phone">
			...
			</strong> <?php echo JText::_('COM_KUNENA_GEN_REPLIES') ?>
		</td>
...
?>
		<tr>
			<td class="ktopicmodule" colspan="<?php echo empty($this->topicActions) ? 5 : 6 ?>">
